...
def find_children():
    ...
    Children include files and folders, and are assinged a non-zero
    numeric value. Children can be filtered by setting the 
...
def list_children():
    ...
    Children are stored in a dicitonary and assinged a numeric value.
    The zero value is set as '...' by default, as a placeholder for the
    parent dir. """

    #Initialise dictionary containing '...' as placeholder for parent
    #dir
    child_dict = dict({0: "..."})
    ...
    #Iterate over the list of children and add to the children
    #dictionary setting the key as the file/folder name and incrementing
    #the value
    n = 1
    for i in dir_list:
        child_dict[n] = i
    ...
    #Return the dictionary of file/folder name keys and number value
    #pairs
    return child_dict
...
def change_dir():
    ...
    Can change to the parent of the current directory, or to a sub
    directory by selecting the assosiated number generated by
    list_children(). If a child that is not a directory is selected the
    relevent exception is caught and an appropriate message displayed.
    """
