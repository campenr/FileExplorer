...
def clean_path(current_dir):
    """Clean path to only show the top three, and bottom two levels.
     ...
     - split_path should be a list of path levels, generated by .split(os.sep)
    """

    split_path = current_dir.split(os.sep)
    if len(split_path) >= 7:
        top_levels = split_path[:3]
        bottom_levels = split_path[-2:]
        return "".join((os.sep.join(top_levels), "{0}...{0}".format(os.sep), os.sep.join(bottom_levels)))
...
def change_dir():
    ...
    cleaned_path = clean_path(current_dir)
    print("\n[Current dir] ", cleaned_path, "\n")
    ...
    dir_number = int(input("\nEnter a number to select the corresponding "
